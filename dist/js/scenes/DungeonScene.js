...
export class DungeonScene extends Phaser.Scene {
    constructor() {
        super(...arguments);
        this.dungeonOffset = 64;
        this.doorSprites = [];
        this.inputStack = [];
        this.pendingLock = false;
        this.currentLevel = 1;
        this.levelTexture = 'master_sheet';
    ...
    create(data) {
        this.currentLevel = (data === null || data === void 0 ? void 0 : data.level) || 1;
        ...
        this.input.keyboard.on('keydown', (event) => {
            ...
            if (dir && !this.inputStack.includes(dir))
                this.inputStack.push(dir);
        });
        this.input.keyboard.on('keyup', (event) => {
            const dir = keyMap[event.keyCode];
            if (dir)
                this.inputStack = this.inputStack.filter(d => d !== dir);
        });
        if (data === null || data === void 0 ? void 0 : data.autoStart) {
            this.startGame();
        }
        else {
    ...
    startGame() {
        ...
        window.playerModel = this.playerModel; // Add this line for debugging
    ...
    popMode() {
        if (this.modeStack.length > 0) {
            ...
            mode === null || mode === void 0 ? void 0 : mode.exit();
        }
    }
    switchMode(mode) {
        this.popMode();
    ...
    loadSkin(skinName) {
        ...
        for (const [animKey, config] of Object.entries(SKIN_CONFIG.anims)) {
            ...
            for (let i = 0; i < config.length; i++) {
                ...
                this.textures.get('master_sheet').add(frameName, 0, startX + (config.start + i) * w, skinY, w, h);
    ...
    loadWeapons() {
        ...
        WEAPON_CONFIG.names.forEach((name, i) => {
            this.textures.get('master_sheet').add(`weapon_${name}`, 0, startX + (i * w), startY, w, h);
        });
    }
    /**
    ...
    loadItems() {
        ...
        ITEM_CONFIG.names.forEach((name, i) => {
            this.textures.get('master_sheet').add(`item_${name}`, 0, startX + (i * w), startY, w, h);
        });
    }
    /**
     ...
     */
    createEnemyAnims(textureKey = 'master_sheet', suffix = '') {
     ...
     */
    createPaletteTexture(sourceKey, newKey, palette, allowedPrefixes) {
        if (this.textures.exists(newKey))
            return;
        const sourceTexture = this.textures.get(sourceKey);
        const src = sourceTexture.getSourceImage();
    ...
    loadRoom(x, y) {
        ...
        const getDoorState = (isOpen) => (!isOpen ? 'solid' : 'open');
    ...
    switchRoom(dx, dy) {
        if (this.dungeonManager.switchRoom(dx, dy)) {
            ...
            if (dy === -1)
                this.playerModel.y = 224;
            else if (dy === 1)
                this.playerModel.y = 32;
            else if (dx === -1)
                this.playerModel.x = 224;
            else if (dx === 1)
                this.playerModel.x = 32;
    ...
    lockRoom() {
        ...
        if (this.entityManager.count("enemy") > 0) { //added check
            if (cell.north)
                this.roomModel.wallTypes.n = 'shut';
            if (cell.south)
                this.roomModel.wallTypes.s = 'shut';
            if (cell.west)
                this.roomModel.wallTypes.w = 'shut';
            if (cell.east)
                this.roomModel.wallTypes.e = 'shut';
            this.drawDoors();
        }
    }
    handleRoomClear() {
        if (this.entityManager.count("enemy") > 0)
            return;
        const cell = this.dungeonManager.currentCell;
        cell.cleared = true;
        if (cell.north)
            this.roomModel.wallTypes.n = 'open';
        if (cell.south)
            this.roomModel.wallTypes.s = 'open';
        if (cell.west)
            this.roomModel.wallTypes.w = 'open';
        if (cell.east)
            this.roomModel.wallTypes.e = 'open';
        this.drawDoors();
        this.hudManager.updateHearts(this.playerModel.hp);
